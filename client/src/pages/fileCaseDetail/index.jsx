...
import {addScore, deleteCollection, saveCollection} from "../../util/userCollection";
...
export default class SentencingDetail extends Component {
  ...
  onShareAppMessage() {
    ...
    addScore();
    return {
      path: `pages/fileCaseDetail/index?id=${id}&criminalLawNumber=${criminalLawNumber}`
    };
  }
  ...
  componentWillMount () {
    ...
    Taro.setNavigationBarTitle({title: `刑法${criminalLawNumber}立案追诉标准`})
    ...
    db.collection('criminal-file-case').where({criminalLaw: criminalLawNumber}).orderBy('effectiveDate', 'desc').get({
      success: (res) => {
        console.log(res.data)
        that.setState({sentencings: res.data, isSentencingLoading: false});
      }
    });
  ...
  renderSentencings = () => {
    ...
    return <View className='sentencings'>
      {sentencings.map((sentencing, index) => {
        const {crime, text, source, link, effectiveDate} = sentencing
        return (<View className='sentencing' key={`sentencing-key-${index}`}>
          ...
          <View className='line link'>
            <DataPopup data={{sourceName: source, link}} type='source' zoomIn={zoomIn} />
          </View>
  ...
  handleCollect = throttle(() => {
    ...
    if (isCollected) {
      ...
      saveCollection(id, 'fileCase', `刑法${criminalLawNumber}立案追诉标准`).then(() => {
        Taro.showToast({
          title: '收藏成功',
          icon: 'none',
          duration: 1000
