const cloud = require('wx-server-sdk')

// https://developers.weixin.qq.com/miniprogram/dev/wxcloud/reference-server-api/database/collection.where.html
cloud.init({
    env: cloud.DYNAMIC_CURRENT_ENV
})

const lawMap = {
    'civil-litigation-explanation-2022-04-10': { '一、管辖': { value: '一、管辖', start: 1, end: 42 },
        '二、回避': { value: '二、回避', start: 43, end: 49 },
        '三、诉讼参加人': { value: '三、诉讼参加人', start: 50, end: 89 },
        '四、证据': { value: '四、证据', start: 90, end: 124 },
        '五、期间和送达': { value: '五、期间和送达', start: 125, end: 141 },
        '六、调解': { value: '六、调解', start: 142, end: 151 },
        '七、保全和先予执行': { value: '七、保全和先予执行', start: 152, end: 173 },
        '八、对妨害民事诉讼的强制措施': { value: '八、对妨害民事诉讼的强制措施', start: 174, end: 193 },
        '九、诉讼费用': { value: '九、诉讼费用', start: 194, end: 207 },
        '十、第一审普通程序': { value: '十、第一审普通程序', start: 208, end: 255 },
        '十一、简易程序': { value: '十一、简易程序', start: 256, end: 270 },
        '十二、简易程序中的小额诉讼': { value: '十二、简易程序中的小额诉讼', start: 271, end: 281 },
        '十三、公益诉讼': { value: '十三、公益诉讼', start: 282, end: 289 },
        '十四、第三人撤销之诉': { value: '十四、第三人撤销之诉', start: 290, end: 301 },
        '十五、执行异议之诉': { value: '十五、执行异议之诉', start: 302, end: 314 },
        '十六、第二审程序': { value: '十六、第二审程序', start: 315, end: 340 },
        '十七、特别程序': { value: '十七、特别程序', start: 341, end: 372 },
        '十八、审判监督程序': { value: '十八、审判监督程序', start: 373, end: 424 },
        '十九、督促程序': { value: '十九、督促程序', start: 425, end: 441 },
        '二十、公示催告程序': { value: '二十、公示催告程序', start: 442, end: 459 },
        '二十一、执行程序': { value: '二十一、执行程序', start: 460, end: 519 },
        '二十二、涉外民事诉讼程序的特别规定': { value: '二十二、涉外民事诉讼程序的特别规定', start: 520, end: 549 },
        '二十三、附则': { value: '二十三、附则', start: 550, end: 550 } },
    'medical-malpractice-regulation':{ '第一章　总则': { value: '第一章　总则', start: 1, end: 4 },
        '第二章　医疗事故的预防与处置': { value: '第二章　医疗事故的预防与处置', start: 5, end: 19 },
        '第三章　医疗事故的技术鉴定': { value: '第三章　医疗事故的技术鉴定', start: 20, end: 34 },
        '第四章　医疗事故的行政处理与监督': { value: '第四章　医疗事故的行政处理与监督', start: 35, end: 45 },
        '第五章　医疗事故的赔偿': { value: '第五章　医疗事故的赔偿', start: 46, end: 52 },
        '第六章　罚则': { value: '第六章　罚则', start: 53, end: 59 },
        '第七章　附则': { value: '第七章　附则', start: 60, end: 63 } },
    'labor-conciliation':{ '第一章　总则': { value: '第一章　总则', start: 1, end: 9 },
        '第二章　调解': { value: '第二章　调解', start: 10, end: 16 },
        '第三章　仲裁': { value: '第三章　仲裁', start: 17, end: 52 },
        '第一节　一般规定': { value: '第一节　一般规定', start: 17, end: 26 },
        '第二节　申请和受理': { value: '第二节　申请和受理', start: 27, end: 30 },
        '第三节　开庭和裁决': { value: '第三节　开庭和裁决', start: 31, end: 51 },
        '第四章　附则': { value: '第四章　附则', start: 52, end: 54 } },
    'fire-fighting': { '第一章　总则': { value: '第一章　总则', start: 1, end: 7 },
        '第二章　火灾预防': { value: '第二章　火灾预防', start: 8, end: 34 },
        '第三章　消防组织': { value: '第三章　消防组织', start: 35, end: 42 },
        '第四章　灭火救援': { value: '第四章　灭火救援', start: 43, end: 51 },
        '第五章　监督检查': { value: '第五章　监督检查', start: 52, end: 57 },
        '第六章　法律责任': { value: '第六章　法律责任', start: 58, end: 72 },
        '第七章　附则': { value: '第七章　附则', start: 73, end: 74 } },
    'civil-law-regulation': { '第一章 任务、适用范围和基本原则': { value: '第一章 任务、适用范围和基本原则', start: 1, end: 17 },
        '第二章 管 辖': { value: '第二章 管 辖', start: 18, end: 39 },
        '第一节 级别管辖': { value: '第一节 级别管辖', start: 18, end: 21 },
        '第二节 地域管辖': { value: '第二节 地域管辖', start: 22, end: 36 },
        '第三节 移送管辖和指定管辖': { value: '第三节 移送管辖和指定管辖', start: 37, end: 39 },
        '第三章 审判组织': { value: '第三章 审判组织', start: 40, end: 46 },
        '第四章 回 避': { value: '第四章 回 避', start: 47, end: 50 },
        '第五章 诉讼参加人': { value: '第五章 诉讼参加人', start: 51, end: 65 },
        '第一节 当事人': { value: '第一节 当事人', start: 51, end: 59 },
        '第二节 诉讼代理人': { value: '第二节 诉讼代理人', start: 60, end: 65 },
        '第六章 证 据': { value: '第六章 证 据', start: 66, end: 84 },
        '第七章 期间、送达': { value: '第七章 期间、送达', start: 85, end: 95 },
        '第一节 期 间': { value: '第一节 期 间', start: 85, end: 86 },
        '第二节 送 达': { value: '第二节 送 达', start: 87, end: 95 },
        '第八章 调 解': { value: '第八章 调 解', start: 96, end: 102 },
        '第九章 保全和先予执行': { value: '第九章 保全和先予执行', start: 103, end: 111 },
        '第十章 对妨害民事诉讼的强制措施': { value: '第十章 对妨害民事诉讼的强制措施', start: 112, end: 120 },
        '第十一章 诉讼费用': { value: '第十一章 诉讼费用', start: 121, end: 121 },
        '第十二章 第一审普通程序': { value: '第十二章 第一审普通程序', start: 122, end: 159 },
        '第一节 起诉和受理': { value: '第一节 起诉和受理', start: 122, end: 127 },
        '第二节 审理前的准备': { value: '第二节 审理前的准备', start: 128, end: 136 },
        '第三节 开庭审理': { value: '第三节 开庭审理', start: 137, end: 152 },
        '第四节 诉讼中止和终结': { value: '第四节 诉讼中止和终结', start: 153, end: 154 },
        '第五节 判决和裁定': { value: '第五节 判决和裁定', start: 155, end: 159 },
        '第十三章 简易程序': { value: '第十三章 简易程序', start: 160, end: 170 },
        '第十四章 第二审程序': { value: '第十四章 第二审程序', start: 171, end: 183 },
        '第十五章 特别程序': { value: '第十五章 特别程序', start: 184, end: 204 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 184, end: 187 },
        '第二节 选民资格案件': { value: '第二节 选民资格案件', start: 188, end: 189 },
        '第三节 宣告失踪、宣告死亡案件': { value: '第三节 宣告失踪、宣告死亡案件', start: 190, end: 193 },
        '第四节 认定公民无民事行为能力、限制民事行为能力案件':
            { value: '第四节 认定公民无民事行为能力、限制民事行为能力案件', start: 194, end: 197 },
        '第五节 认定财产无主案件': { value: '第五节 认定财产无主案件', start: 198, end: 200 },
        '第六节 确认调解协议案件': { value: '第六节 确认调解协议案件', start: 201, end: 202 },
        '第七节 实现担保物权案件': { value: '第七节 实现担保物权案件', start: 203, end: 204 },
        '第十六章 审判监督程序': { value: '第十六章 审判监督程序', start: 205, end: 220 },
        '第十七章 督促程序': { value: '第十七章 督促程序', start: 221, end: 224 },
        '第十八章 公示催告程序': { value: '第十八章 公示催告程序', start: 225, end: 230 },
        '第十九章 一般规定': { value: '第十九章 一般规定', start: 231, end: 242 },
        '第二十章 执行的申请和移送': { value: '第二十章 执行的申请和移送', start: 243, end: 247 },
        '第二十一章 执行措施': { value: '第二十一章 执行措施', start: 248, end: 262 },
        '第二十二章 执行中止和终结': { value: '第二十二章 执行中止和终结', start: 263, end: 265 },
        '第二十三章 一般原则': { value: '第二十三章 一般原则', start: 266, end: 271 },
        '第二十四章 管 辖': { value: '第二十四章 管 辖', start: 272, end: 273 },
        '第二十五章 送达、期间': { value: '第二十五章 送达、期间', start: 274, end: 277 },
        '第二十六章 仲 裁': { value: '第二十六章 仲 裁', start: 278, end: 282 },
        '第二十七章 司法协助': { value: '第二十七章 司法协助', start: 283, end: 291 } },
    'public-interest-rule':{ '第一章　总　则': { value: '第一章　总　则', start: 1, end: 12 },
        '第二章　一般规定': { value: '第二章　一般规定', start: 13, end: 66 },
        '第一节　管　辖': { value: '第一节　管　辖', start: 13, end: 18 },
        '第二节　回　避': { value: '第二节　回　避', start: 19, end: 23 },
        '第三节　立　案': { value: '第三节　立　案', start: 24, end: 31 },
        '第四节　调　查': { value: '第四节　调　查', start: 32, end: 45 },
        '第五节　提起诉讼': { value: '第五节　提起诉讼', start: 46, end: 48 },
        '第六节　出席第一审法庭': { value: '第六节　出席第一审法庭', start: 49, end: 57 },
        '第七节　上　诉': { value: '第七节　上　诉', start: 58, end: 63 },
        '第八节　诉讼监督': { value: '第八节　诉讼监督', start: 64, end: 66 },
        '第三章　行政公益诉讼': { value: '第三章　行政公益诉讼', start: 67, end: 84 },
        '第一节　立案与调查': { value: '第一节　立案与调查', start: 67, end: 74 },
        '第二节　检察建议': { value: '第二节　检察建议', start: 75, end: 80 },
        '第三节　提起诉讼': { value: '第三节　提起诉讼', start: 81, end: 84 },
        '第四章　民事公益诉讼': { value: '第四章　民事公益诉讼', start: 85, end: 103 },
        '第一节　立案与调查_': { value: '第一节　立案与调查_', start: 85, end: 90 },
        '第二节　公　告': { value: '第二节　公　告', start: 91, end: 95 },
        '第四节　支持起诉': { value: '第四节　支持起诉', start: 100, end: 103 },
        '第五章　其他规定': { value: '第五章　其他规定', start: 104, end: 107 },
        '第六章　附　则': { value: '第六章　附　则', start: 108, end: 112 } },
    'civil-litigation-explanation': { '一、管辖': { value: '一、管辖', start: 1, end: 42 },
        '二、回避': { value: '二、回避', start: 43, end: 49 },
        '三、诉讼参加人': { value: '三、诉讼参加人', start: 50, end: 89 },
        '四、证据': { value: '四、证据', start: 90, end: 124 },
        '五、期间和送达': { value: '五、期间和送达', start: 125, end: 141 },
        '六、调解': { value: '六、调解', start: 142, end: 151 },
        '七、保全和先予执行': { value: '七、保全和先予执行', start: 152, end: 173 },
        '八、对妨害民事诉讼的强制措施': { value: '八、对妨害民事诉讼的强制措施', start: 174, end: 193 },
        '九、诉讼费用': { value: '九、诉讼费用', start: 194, end: 207 },
        '十、第一审普通程序': { value: '十、第一审普通程序', start: 208, end: 155 },
        '十一、简易程序': { value: '十一、简易程序', start: 256, end: 270 },
        '十二、简易程序中的小额诉讼': { value: '十二、简易程序中的小额诉讼', start: 271, end: 283 },
        '十三、公益诉讼': { value: '十三、公益诉讼', start: 284, end: 291 },
        '十四、第三人撤销之诉': { value: '十四、第三人撤销之诉', start: 292, end: 303 },
        '十五、执行异议之诉': { value: '十五、执行异议之诉', start: 304, end: 316 },
        '十六、第二审程序': { value: '十六、第二审程序', start: 317, end: 342 },
        '十七、特别程序': { value: '十七、特别程序', start: 343, end: 374 },
        '十八、审判监督程序': { value: '十八、审判监督程序', start: 375, end: 426 },
        '十九、督促程序': { value: '十九、督促程序', start: 427, end: 443 },
        '二十、公示催告程序': { value: '二十、公示催告程序', start: 444, end: 461 },
        '二十一、执行程序': { value: '二十一、执行程序', start: 462, end: 521 },
        '二十二、涉外民事诉讼程序的特别规定': { value: '二十二、涉外民事诉讼程序的特别规定', start: 522, end: 551 },
        '二十三、附则': { value: '二十三、附则', start: 552, end: 552 } },
    'criminal-litigation-explanation': { '第一章 管辖': { value: '第一章 管辖', start: 1, end: 26 },
        '第二章 回避': { value: '第二章 回避', start: 27, end: 39 },
        '第三章 辩护与代理': { value: '第三章 辩护与代理', start: 40, end: 68 },
        '第四章 证据': { value: '第四章 证据', start: 69, end: 146 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 546, end: 562 },
        '第二节 物证、书证的审查与认定': { value: '第二节 物证、书证的审查与认定', start: 82, end: 86 },
        '第三节 证人证言、被害人陈述的审查与认定': { value: '第三节 证人证言、被害人陈述的审查与认定', start: 87, end: 92 },
        '第四节 被告人供述和辩解的审查与认定': { value: '第四节 被告人供述和辩解的审查与认定', start: 93, end: 96 },
        '第五节 鉴定意见的审查与认定': { value: '第五节 鉴定意见的审查与认定', start: 97, end: 101 },
        '第六节 勘验、检查、辨认、侦查实验等笔录的审查与认定':
            { value: '第六节 勘验、检查、辨认、侦查实验等笔录的审查与认定', start: 102, end: 107 },
        '第七节 视听资料、电子数据的审查与认定': { value: '第七节 视听资料、电子数据的审查与认定', start: 108, end: 115 },
        '第八节 技术调查、侦查证据的审查与认定': { value: '第八节 技术调查、侦查证据的审查与认定', start: 116, end: 122 },
        '第九节 非法证据排除': { value: '第九节 非法证据排除', start: 123, end: 138 },
        '第十节 证据的综合审查与运用': { value: '第十节 证据的综合审查与运用', start: 139, end: 146 },
        '第五章 强制措施': { value: '第五章 强制措施', start: 147, end: 174 },
        '第六章 附带民事诉讼': { value: '第六章 附带民事诉讼', start: 175, end: 201 },
        '第七章 期间、送达、审理期限': { value: '第七章 期间、送达、审理期限', start: 202, end: 211 },
        '第八章 审判组织': { value: '第八章 审判组织', start: 212, end: 217 },
        '第九章 公诉案件第一审普通程序': { value: '第九章 公诉案件第一审普通程序', start: 218, end: 315 },
        '第一节 审查受理与庭前准备': { value: '第一节 审查受理与庭前准备', start: 218, end: 225 },
        '第二节 庭前会议与庭审衔接': { value: '第二节 庭前会议与庭审衔接', start: 226, end: 233 },
        '第三节 宣布开庭与法庭调查': { value: '第三节 宣布开庭与法庭调查', start: 234, end: 279 },
        '第四节 法庭辩论与最后陈述': { value: '第四节 法庭辩论与最后陈述', start: 280, end: 290 },
        '第五节 评议案件与宣告判决': { value: '第五节 评议案件与宣告判决', start: 291, end: 304 },
        '第六节 法庭纪律与其他规定': { value: '第六节 法庭纪律与其他规定', start: 305, end: 315 },
        '第十章 自诉案件第一审程序': { value: '第十章 自诉案件第一审程序', start: 316, end: 334 },
        '第十一章 单位犯罪案件的审理': { value: '第十一章 单位犯罪案件的审理', start: 335, end: 346 },
        '第十二章 认罪认罚案件的审理': { value: '第十二章 认罪认罚案件的审理', start: 347, end: 358 },
        '第十三章 简易程序': { value: '第十三章 简易程序', start: 359, end: 368 },
        '第十四章 速裁程序': { value: '第十四章 速裁程序', start: 369, end: 377 },
        '第十五章 第二审程序': { value: '第十五章 第二审程序', start: 378, end: 413 },
        '第十六章 在法定刑以下判处刑罚和特殊假释的核准': { value: '第十六章 在法定刑以下判处刑罚和特殊假释的核准', start: 414, end: 422 },
        '第十七章 死刑复核程序': { value: '第十七章 死刑复核程序', start: 423, end: 436 },
        '第十八章 涉案财物处理': { value: '第十八章 涉案财物处理', start: 437, end: 450 },
        '第十九章 审判监督程序': { value: '第十九章 审判监督程序', start: 451, end: 474 },
        '第二十章 涉外刑事案件的审理和刑事司法协助': { value: '第二十章 涉外刑事案件的审理和刑事司法协助', start: 475, end: 496 },
        '第一节 涉外刑事案件的审理': { value: '第一节 涉外刑事案件的审理', start: 475, end: 490 },
        '第二节 刑事司法协助': { value: '第二节 刑事司法协助', start: 491, end: 496 },
        '第二十一章 执行程序': { value: '第二十一章 执行程序', start: 497, end: 545 },
        '第一节 死刑的执行': { value: '第一节 死刑的执行', start: 497, end: 510 },
        '第二节 死刑缓期执行、无期徒刑、有期徒刑、拘役的交付执行':
            { value: '第二节 死刑缓期执行、无期徒刑、有期徒刑、拘役的交付执行', start: 511, end: 518 },
        '第三节 管制、缓刑、剥夺政治权利的交付执行': { value: '第三节 管制、缓刑、剥夺政治权利的交付执行', start: 519, end: 520 },
        '第四节 刑事裁判涉财产部分和附带民事裁判的执行': { value: '第四节 刑事裁判涉财产部分和附带民事裁判的执行', start: 521, end: 532 },
        '第五节 减刑、假释案件的审理': { value: '第五节 减刑、假释案件的审理', start: 533, end: 541 },
        '第六节 缓刑、假释的撤销': { value: '第六节 缓刑、假释的撤销', start: 542, end: 545 },
        '第二十二章 未成年人刑事案件诉讼程序': { value: '第二十二章 未成年人刑事案件诉讼程序', start: 546, end: 586 },
        '第二节 开庭准备': { value: '第二节 开庭准备', start: 563, end: 570 },
        '第三节 审判': { value: '第三节 审判', start: 571, end: 579 },
        '第四节 执行': { value: '第四节 执行', start: 580, end: 586 },
        '第二十三章 当事人和解的公诉案件诉讼程序': { value: '第二十三章 当事人和解的公诉案件诉讼程序', start: 587, end: 597 },
        '第二十四章 缺席审判程序': { value: '第二十四章 缺席审判程序', start: 598, end: 608 },
        '第二十五章 犯罪嫌疑人、被告人逃匿、死亡案件违法所得的没收程序':
            { value: '第二十五章 犯罪嫌疑人、被告人逃匿、死亡案件违法所得的没收程序',
                start: 609,
                end: 629 },
        '第二十六章 依法不负刑事责任的精神病人的强制医疗程序':
            { value: '第二十六章 依法不负刑事责任的精神病人的强制医疗程序', start: 630, end: 649 },
        '第二十七章 附则': { value: '第二十七章 附则', start: 650, end: 655 } },
    'road-safe-violation-handling': { '第一章 总则': { value: '第一章 总则', start: 1, end: 3 },
        '第二章 管辖': { value: '第二章 管辖', start: 4, end: 6 },
        '第三章 调查取证': { value: '第三章 调查取证', start: 7, end: 23 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 7, end: 14 },
        '第二节 交通技术监控': { value: '第二节 交通技术监控', start: 15, end: 23 },
        '第四章 行政强制措施适用': { value: '第四章 行政强制措施适用', start: 24, end: 41 },
        '第五章 行政处罚': { value: '第五章 行政处罚', start: 42, end: 62 },
        '第一节 行政处罚的决定': { value: '第一节 行政处罚的决定', start: 42, end: 58 },
        '第二节 行政处罚的执行': { value: '第二节 行政处罚的执行', start: 59, end: 62 },
        '第六章 执法监督': { value: '第六章 执法监督', start: 63, end: 66 },
        '第七章 其他规定': { value: '第七章 其他规定', start: 67, end: 76 },
        '第八章 附则': { value: '第八章 附则', start: 77, end: 83 } },
    'police-admin-regulation': {
        '第一章 总则': { start: 1, end: 9 },
        '第二章 管辖': { start: 10, end: 16 },
        '第三章 回避': { start: 17, end: 25 },
        '第四章 证据': { start: 26, end: 34 },
        '第五章 期间与送达': { start: 35, end: 36 },
        '第六章 简易程序和快速办理': { value: '第六章 简易程序和快速办理', start: 37, end: 48 },
        '第一节 简易程序': { start: 37, end: 39 },
        '第二节 快速办理': { start: 40, end: 48 },
        '第七章 调查取证': { value: '第七章 调查取证', start: 49, end: 122 },
        '第一节 一般规定': { start: 198, end: 213 },
        '第二节 受案': { start: 60, end: 65 },
        '第三节 询问': { start: 66, end: 80 },
        '第四节 勘验、检查': { start: 81, end: 86 },
        '第五节 鉴定': { start: 87, end: 100 },
        '第六节 辨认': { start: 101, end: 106 },
        '第七节 证据保全': { start: 107, end: 116 },
        '第八节 办案协作': { start: 117, end: 122 },
        '第八章 听证程序': { value: '第八章 听证程序', start: 123, end: 153 },
        '第二节 听证人员和听证参加人': { start: 127, end: 131 },
        '第三节 听证的告知、申请和受理': { start: 132, end: 136 },
        '第四节 听证的举行': { start: 137, end: 153 },
        '第九章 行政处理决定': { value: '第九章 行政处理决定', start: 154, end: 177 },
        '第一节 行政处罚的适用': { start: 154, end: 164 },
        '第二节 行政处理的决定': { start: 165, end: 177 },
        '第十章 治安调解': { value: '第十章 治安调解', start: 178, end: 186 },
        '第十一章 涉案财物的管理和处理': { value: '第十一章 涉案财物的管理和处理', start: 187, end: 197 },
        '第十二章 执行': { value: '第十二章 执行', start: 198, end: 237 },
        '第二节 罚款的执行': { start: 214, end: 219 },
        '第三节 行政拘留的执行': { start: 220, end: 233 },
        '第四节 其他处理决定的执行': { start: 234, end: 237 },
        '第十三章 涉外行政案件的办理': { value: '第十三章 涉外行政案件的办理', start: 238, end: 257 },
        '第十四章 案件终结': { value: '第十四章 案件终结', start: 258, end: 262 },
        '第十五章 附则': { value: '第十五章 附则', start: 263, end: 266 } },
    'road-safe-regulation':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 3 },
        '第二章 车辆和驾驶人': { value: '第二章 车辆和驾驶人', start: 4, end: 28 },
        '第一节 机动车': { value: '第一节 机动车', start: 4, end: 18 },
        '第二节 机动车驾驶人': { value: '第二节 机动车驾驶人', start: 19, end: 28 },
        '第三章 道路通行条件': { value: '第三章 道路通行条件', start: 29, end: 37 },
        '第四章 道路通行规定': { value: '第四章 道路通行规定', start: 38, end: 85 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 38, end: 43 },
        '第二节 机动车通行规定': { value: '第二节 机动车通行规定', start: 44, end: 67 },
        '第三节 非机动车通行规定': { value: '第三节 非机动车通行规定', start: 68, end: 73 },
        '第四节 行人和乘车人通行规定': { value: '第四节 行人和乘车人通行规定', start: 74, end: 77 },
        '第五节 高速公路的特别规定': { value: '第五节 高速公路的特别规定', start: 78, end: 85 },
        '第五章 交通事故处理': { value: '第五章 交通事故处理', start: 86, end: 97 },
        '第六章 执法监督': { value: '第六章 执法监督', start: 98, end: 101 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 102, end: 110 },
        '第八章 附则': { value: '第八章 附则', start: 111, end: 115 } },
    'road-safe-law': { '第一章 总则': { value: '第一章 总则', start: 1, end: 7 },
        '第二章 车辆和驾驶人': { value: '第二章 车辆和驾驶人', start: 8, end: 24 },
        '第一节 机动车、非机动车': { value: '第一节 机动车、非机动车', start: 8, end: 18 },
        '第二节 机动车驾驶人': { value: '第二节 机动车驾驶人', start: 19, end: 24 },
        '第三章 道路通行条件': { value: '第三章 道路通行条件', start: 25, end: 34 },
        '第四章 道路通行规定': { value: '第四章 道路通行规定', start: 35, end: 69 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 35, end: 41 },
        '第二节 机动车通行规定': { value: '第二节 机动车通行规定', start: 42, end: 56 },
        '第三节 非机动车通行规定': { value: '第三节 非机动车通行规定', start: 57, end: 60 },
        '第四节 行人和乘车人通行规定': { value: '第四节 行人和乘车人通行规定', start: 61, end: 66 },
        '第五节 高速公路的特别规定': { value: '第五节 高速公路的特别规定', start: 67, end: 69 },
        '第五章 交通事故处理': { value: '第五章 交通事故处理', start: 70, end: 77 },
        '第六章 执法监督': { value: '第六章 执法监督', start: 78, end: 86 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 87, end: 118 },
        '第八章 附则': { value: '第八章 附则', start: 119, end: 124 } },
    'public-order-admin-penalty-law': { '第一章 总则': { value: '第一章 总则', start: 1, end: 9 },
        '第二章 处罚的种类和适用': { value: '第二章 处罚的种类和适用', start: 10, end: 22 },
        '第三章 违反治安管理的行为和处罚': { value: '第三章 违反治安管理的行为和处罚', start: 23, end: 76 },
        '第一节 扰乱公共秩序的行为和处罚': { value: '第一节 扰乱公共秩序的行为和处罚', start: 23, end: 29 },
        '第二节 妨害公共安全的行为和处罚': { value: '第二节 妨害公共安全的行为和处罚', start: 30, end: 39 },
        '第三节 侵犯人身权利、财产权利的行为和处罚': { value: '第三节 侵犯人身权利、财产权利的行为和处罚', start: 40, end: 49 },
        '第四节 妨害社会管理的行为和处罚': { value: '第四节 妨害社会管理的行为和处罚', start: 50, end: 76 },
        '第四章 处罚程序': { value: '第四章 处罚程序', start: 77, end: 111 },
        '第一节 调查': { value: '第一节 调查', start: 77, end: 90 },
        '第二节 决定': { value: '第二节 决定', start: 91, end: 102 },
        '第三节 执行': { value: '第三节 执行', start: 103, end: 111 },
        '第五章 执法监督': { value: '第五章 执法监督', start: 112, end: 117 },
        '第六章 附则': { value: '第六章 附则', start: 118, end: 119 } },
    'public-order-admin-penalty-law-2026': { '第一章 总则': { value: '第一章 总则', start: 1, end: 9 },
        '第二章 处罚的种类和适用': { value: '第二章 处罚的种类和适用', start: 10, end: 25 },
        '第三章 违反治安管理的行为和处罚': { value: '第三章 违反治安管理的行为和处罚', start: 26, end: 89 },
        '第一节 扰乱公共秩序的行为和处罚': { value: '第一节 扰乱公共秩序的行为和处罚', start: 26, end: 35 },
        '第二节 妨害公共安全的行为和处罚': { value: '第二节 妨害公共安全的行为和处罚', start: 36, end: 46 },
        '第三节 侵犯人身权利、财产权利的行为和处罚': { value: '第三节 侵犯人身权利、财产权利的行为和处罚', start: 47, end: 60 },
        '第四节 妨害社会管理的行为和处罚': { value: '第四节 妨害社会管理的行为和处罚', start: 61, end: 89 },
        '第四章 处罚程序': { value: '第四章 处罚程序', start: 90, end: 130 },
        '第一节 调查': { value: '第一节 调查', start: 90, end: 108 },
        '第二节 决定': { value: '第二节 决定', start: 109, end: 121 },
        '第三节 执行': { value: '第三节 执行', start: 122, end: 130 },
        '第五章 执法监督': { value: '第五章 执法监督', start: 131, end: 140 },
        '第六章 附则': { value: '第六章 附则', start: 141, end: 144 } },
    'admin-allow-law': { '第一章 总则': { value: '第一章 总则', start: 1, end: 10 },
        '第二章 行政许可的设定': { value: '第二章 行政许可的设定', start: 11, end: 21 },
        '第三章 行政许可的实施机关': { value: '第三章 行政许可的实施机关', start: 22, end: 28 },
        '第四章 行政许可的实施程序': { value: '第四章 行政许可的实施程序', start: 29, end: 57 },
        '第一节 申请与受理': { value: '第一节 申请与受理', start: 29, end: 33 },
        '第二节 审查与决定': { value: '第二节 审查与决定', start: 34, end: 41 },
        '第三节 期限': { value: '第三节 期限', start: 42, end: 45 },
        '第四节 听证': { value: '第四节 听证', start: 46, end: 48 },
        '第五节 变更与延续': { value: '第五节 变更与延续', start: 49, end: 50 },
        '第六节 特别规定': { value: '第六节 特别规定', start: 51, end: 57 },
        '第五章 行政许可的费用': { value: '第五章 行政许可的费用', start: 58, end: 59 },
        '第六章 监督检查': { value: '第六章 监督检查', start: 60, end: 70 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 71, end: 81 },
        '第八章 附则': { value: '第八章 附则', start: 82, end: 83 } },
    'admin-reconsider-regulation': { '第一章 总则': { value: '第一章 总则', start: 1, end: 4 },
        '第二章 行政复议申请': { value: '第二章 行政复议申请', start: 5, end: 26 },
        '第一节 申请人': { value: '第一节 申请人', start: 5, end: 10 },
        '第二节 被申请人': { value: '第二节 被申请人', start: 11, end: 14 },
        '第三节 行政复议申请期限': { value: '第三节 行政复议申请期限', start: 15, end: 17 },
        '第四节 行政复议申请的提出': { value: '第四节 行政复议申请的提出', start: 18, end: 26 },
        '第三章 行政复议受理': { value: '第三章 行政复议受理', start: 27, end: 31 },
        '第四章 行政复议决定': { value: '第四章 行政复议决定', start: 32, end: 52 },
        '第五章 行政复议指导和监督': { value: '第五章 行政复议指导和监督', start: 53, end: 61 },
        '第六章 法律责任': { value: '第六章 法律责任', start: 62, end: 65 },
        '第七章 附则': { value: '第七章 附则', start: 66, end: 66 } },
    'admin-reconsider-law': { '第一章 总则': { value: '第一章 总则', start: 1, end: 5 },
        '第二章 行政复议范围': { value: '第二章 行政复议范围', start: 6, end: 8 },
        '第三章 行政复议申请': { value: '第三章 行政复议申请', start: 9, end: 16 },
        '第四章 行政复议受理': { value: '第四章 行政复议受理', start: 17, end: 21 },
        '第五章 行政复议决定': { value: '第五章 行政复议决定', start: 22, end: 33 },
        '第六章 法律责任': { value: '第六章 法律责任', start: 34, end: 38 },
        '第七章 附则': { value: '第七章 附则', start: 39, end: 43 } },
    'admin-punish-law': { '第一章 总则': { value: '第一章 总则', start: 1, end: 8 },
        '第二章 行政处罚的种类和设定': { value: '第二章 行政处罚的种类和设定', start: 9, end: 16 },
        '第三章 行政处罚的实施机关': { value: '第三章 行政处罚的实施机关', start: 17, end: 21 },
        '第四章 行政处罚的管辖和适用': { value: '第四章 行政处罚的管辖和适用', start: 22, end: 38 },
        '第五章 行政处罚的决定': { value: '第五章 行政处罚的决定', start: 39, end: 65 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 39, end: 50 },
        '第二节 简易程序': { value: '第二节 简易程序', start: 51, end: 53 },
        '第三节 普通程序': { value: '第三节 普通程序', start: 54, end: 62 },
        '第四节 听证程序': { value: '第四节 听证程序', start: 63, end: 65 },
        '第六章 行政处罚的执行': { value: '第六章 行政处罚的执行', start: 66, end: 75 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 76, end: 83 },
        '第八章 附则': { value: '第八章 附则', start: 84, end: 86 } },
    'admin-force-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 8 },
        '第二章 行政强制的种类和设定': { value: '第二章 行政强制的种类和设定', start: 9, end: 15 },
        '第三章 行政强制措施实施程序': { value: '第三章 行政强制措施实施程序', start: 16, end: 33 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 34, end: 44 },
        '第二节 查封、扣押': { value: '第二节 查封、扣押', start: 22, end: 28 },
        '第三节 冻结': { value: '第三节 冻结', start: 29, end: 33 },
        '第四章 行政机关强制执行程序': { value: '第四章 行政机关强制执行程序', start: 34, end: 52 },
        '第二节 金钱给付义务的执行': { value: '第二节 金钱给付义务的执行', start: 45, end: 49 },
        '第三节 代履行': { value: '第三节 代履行', start: 50, end: 52 },
        '第五章 申请人民法院强制执行': { value: '第五章 申请人民法院强制执行', start: 53, end: 60 },
        '第六章 法律责任': { value: '第六章 法律责任', start: 61, end: 68 },
        '第七章 附则': { value: '第七章 附则', start: 69, end: 71 } },
    'admin-litigation-explaination':{ '一、受案范围': { value: '一、受案范围', start: 1, end: 2 },
        '二、管辖': { value: '二、管辖', start: 3, end: 11 },
        '三、诉讼参加人': { value: '三、诉讼参加人', start: 12, end: 33 },
        '四、证据': { value: '四、证据', start: 34, end: 47 },
        '五、期间、送达': { value: '五、期间、送达', start: 48, end: 52 },
        '六、起诉与受理': { value: '六、起诉与受理', start: 53, end: 70 },
        '七、审理与判决': { value: '七、审理与判决', start: 71, end: 127 },
        '八、行政机关负责人出庭应诉': { value: '八、行政机关负责人出庭应诉', start: 128, end: 132 },
        '九、复议机关作共同被告': { value: '九、复议机关作共同被告', start: 133, end: 136 },
        '十、相关民事争议的一并审理': { value: '十、相关民事争议的一并审理', start: 137, end: 144 },
        '十一、规范性文件的一并审查': { value: '十一、规范性文件的一并审查', start: 145, end: 151 },
        '十二、执行': { value: '十二、执行', start: 152, end: 161 },
        '十三、附则': { value: '十三、附则', start: 162, end: 163 } },
    'admin-litigation-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 11 },
        '第二章 受案范围': { value: '第二章 受案范围', start: 12, end: 13 },
        '第三章 管辖': { value: '第三章 管辖', start: 14, end: 24 },
        '第四章 诉讼参加人': { value: '第四章 诉讼参加人', start: 25, end: 32 },
        '第五章 证据': { value: '第五章 证据', start: 33, end: 43 },
        '第六章 起诉和受理': { value: '第六章 起诉和受理', start: 44, end: 53 },
        '第七章 审理和判决': { value: '第七章 审理和判决', start: 54, end: 93 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 54, end: 66 },
        '第二节 第一审普通程序': { value: '第二节 第一审普通程序', start: 67, end: 81 },
        '第三节 简易程序': { value: '第三节 简易程序', start: 82, end: 84 },
        '第四节 第二审程序': { value: '第四节 第二审程序', start: 85, end: 89 },
        '第五节 审判监督程序': { value: '第五节 审判监督程序', start: 90, end: 93 },
        '第八章 执行': { value: '第八章 执行', start: 94, end: 97 },
        '第九章 涉外行政诉讼': { value: '第九章 涉外行政诉讼', start: 98, end: 100 },
        '第十章 附则': { value: '第十章 附则', start: 101, end: 103 } },
    'company-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 22 },
        '第二章 有限责任公司的设立和组织机构': { value: '第二章 有限责任公司的设立和组织机构', start: 23, end: 70 },
        '第一节 设立': { value: '第一节 设立', start: 76, end: 97 },
        '第二节 组织机构': { value: '第二节 组织机构', start: 36, end: 56 },
        '第三节 一人有限责任公司的特别规定': { value: '第三节 一人有限责任公司的特别规定', start: 57, end: 63 },
        '第四节 国有独资公司的特别规定': { value: '第四节 国有独资公司的特别规定', start: 64, end: 70 },
        '第三章 有限责任公司的股权转让': { value: '第三章 有限责任公司的股权转让', start: 71, end: 75 },
        '第四章 股份有限公司的设立和组织机构': { value: '第四章 股份有限公司的设立和组织机构', start: 76, end: 124 },
        '第二节 股东大会': { value: '第二节 股东大会', start: 98, end: 107 },
        '第三节 董事会、经理': { value: '第三节 董事会、经理', start: 108, end: 116 },
        '第四节 监事会': { value: '第四节 监事会', start: 117, end: 119 },
        '第五节 上市公司组织机构的特别规定': { value: '第五节 上市公司组织机构的特别规定', start: 120, end: 124 },
        '第五章 股份有限公司的股份发行和转让': { value: '第五章 股份有限公司的股份发行和转让', start: 125, end: 145 },
        '第一节 股份发行': { value: '第一节 股份发行', start: 125, end: 136 },
        '第二节 股份转让': { value: '第二节 股份转让"', start: 137, end: 145 },
        '第六章 公司董事、监事、高级管理人员的资格和义务': { value: '第六章 公司董事、监事、高级管理人员的资格和义务', start: 146, end: 152 },
        '第七章 公司债券': { value: '第七章 公司债券', start: 153, end: 162 },
        '第八章 公司财务、会计': { value: '第八章 公司财务、会计', start: 163, end: 171 },
        '第九章 公司合并、分立、增资、减资': { value: '第九章 公司合并、分立、增资、减资', start: 172, end: 179 },
        '第十章 公司解散和清算': { value: '第十章 公司解散和清算', start: 180, end: 190 },
        '第十一章 外国公司的分支机构': { value: '第十一章 外国公司的分支机构', start: 191, end: 197 },
        '第十二章 法律责任': { value: '第十二章 法律责任', start: 198, end: 215 },
        '第十三章 附则': { value: '第十三章 附则', start: 216, end: 218 } },
    'company-law-2024': { '第一章 总则': { value: '第一章 总则', start: 1, end: 28 },
        '第二章 公司登记': { value: '第二章 公司登记', start: 29, end: 41 },
        '第三章 有限责任公司的设立和组织机构': { value: '第三章 有限责任公司的设立和组织机构', start: 42, end: 83 },
        '第一节 设立': { value: '第一节 设立', start: 42, end: 57 },
        '第二节 组织机构': { value: '第二节 组织机构', start: 58, end: 83 },
        '第四章 有限责任公司的股权转让': { value: '第四章 有限责任公司的股权转让', start: 84, end: 90 },
        '第五章 股份有限公司的设立和组织机构': { value: '第五章 股份有限公司的设立和组织机构', start: 91, end: 141 },
        '第二节 股东会': { value: '第二节 股东会', start: 111, end: 119 },
        '第三节 董事会、经理': { value: '第三节 董事会、经理', start: 120, end: 129 },
        '第四节 监事会': { value: '第四节 监事会', start: 130, end: 133 },
        '第五节 上市公司组织机构的特别规定': { value: '第五节 上市公司组织机构的特别规定', start: 134, end: 141 },
        '第六章 股份有限公司的股份发行和转让': { value: '第六章 股份有限公司的股份发行和转让', start: 142, end: 167 },
        '第一节 股份发行': { value: '第一节 股份发行', start: 142, end: 156 },
        '第二节 股份转让': { value: '第二节 股份转让', start: 157, end: 167 },
        '第七章 国家出资公司组织机构的特别规定': { value: '第七章 国家出资公司组织机构的特别规定', start: 168, end: 177 },
        '第八章 公司董事、监事、高级管理人员的资格和义务': { value: '第八章 公司董事、监事、高级管理人员的资格和义务', start: 178, end: 193 },
        '第九章 公司债券': { value: '第九章 公司债券', start: 194, end: 206 },
        '第十章 公司财务、会计': { value: '第十章 公司财务、会计', start: 207, end: 217 },
        '第十一章 公司合并、分立、增资、减资': { value: '第十一章 公司合并、分立、增资、减资', start: 218, end: 228 },
        '第十二章 公司解散和清算': { value: '第十二章 公司解散和清算', start: 229, end: 242 },
        '第十三章 外国公司的分支机构': { value: '第十三章 外国公司的分支机构', start: 243, end: 259 },
        '第十四章 法律责任': { value: '第十四章 法律责任', start: 250, end: 264 },
        '第十五章 附则': { value: '第十五章 附则', start: 265, end: 266 } },

    'labor-contract-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 6 },
        '第二章 劳动合同的订立': { value: '第二章 劳动合同的订立', start: 7, end: 28 },
        '第三章 劳动合同的履行和变更': { value: '第三章 劳动合同的履行和变更', start: 29, end: 35 },
        '第四章 劳动合同的解除和终止': { value: '第四章 劳动合同的解除和终止', start: 36, end: 50 },
        '第五章 特别规定': { value: '第五章 特别规定', start: 51, end: 72 },
        '第一节 集体合同': { value: '第一节 集体合同', start: 51, end: 56 },
        '第二节 劳务派遣': { value: '第二节 劳务派遣', start: 57, end: 67 },
        '第三节 非全日制用工': { value: '第三节 非全日制用工', start: 68, end: 72 },
        '第六章 监督检查': { value: '第六章 监督检查', start: 73, end: 79 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 80, end: 95 },
        '第八章 附则': { value: '第八章 附则', start: 96, end: 98 } },
    'labor-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 9 },
        '第二章 促进就业': { value: '第二章 促进就业', start: 10, end: 15 },
        '第三章 劳动合同和集体合同': { value: '第三章 劳动合同和集体合同', start: 16, end: 35 },
        '第四章 工作时间和休息休假': { value: '第四章 工作时间和休息休假', start: 36, end: 45 },
        '第五章 工资': { value: '第五章 工资', start: 46, end: 51 },
        '第六章 劳动安全卫生': { value: '第六章 劳动安全卫生', start: 52, end: 57 },
        '第七章 女职工和未成年工特殊保护': { value: '第七章 女职工和未成年工特殊保护', start: 58, end: 65 },
        '第八章 职业培训': { value: '第八章 职业培训', start: 66, end: 69 },
        '第九章 社会保险和福利': { value: '第九章 社会保险和福利', start: 70, end: 76 },
        '第十章 劳动争议': { value: '第十章 劳动争议', start: 77, end: 84 },
        '第十一章 监督检查': { value: '第十一章 监督检查', start: 85, end: 88 },
        '第十二章 法律责任': { value: '第十二章 法律责任', start: 89, end: 105 },
        '第十三章 附则': { value: '第十三章 附则', start: 106, end: 107 } },
    'anti-drug-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 10 },
        '第二章 宣传教育': { value: '第二章 宣传教育', start: 11, end: 18 },
        '第三章 毒品管制': { value: '第三章 毒品管制', start: 19, end: 30 },
        '第四章 戒毒措施': { value: '第四章 戒毒措施', start: 31, end: 52 },
        '第五章 国际合作': { value: '第五章 国际合作', start: 53, end: 58 },
        '第六章 法律责任': { value: '第六章 法律责任', start: 59, end: 70 },
        '第七章 附则': { value: '第七章 附则', start: 71, end: 71 } },
    'anti-terrorism-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 11 },
        '第二章 恐怖活动组织和人员的认定': { value: '第二章 恐怖活动组织和人员的认定', start: 12, end: 16 },
        '第三章 安全防范': { value: '第三章 安全防范', start: 17, end: 42 },
        '第四章 情报信息': { value: '第四章 情报信息', start: 43, end: 48 },
        '第五章 调查': { value: '第五章 调查', start: 49, end: 54 },
        '第六章 应对处置': { value: '第六章 应对处置', start: 55, end: 67 },
        '第七章 国际合作': { value: '第七章 国际合作', start: 68, end: 72 },
        '第八章 保障措施': { value: '第八章 保障措施', start: 73, end: 78 },
        '第九章 法律责任': { value: '第九章 法律责任', start: 79, end: 96 },
        '第十章 附则': { value: '第十章 附则', start: 97, end: 97 } },
    'supervision-law':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 6 },
        '第二章 监察机关及其职责': { value: '第二章 监察机关及其职责', start: 7, end: 14 },
        '第三章 监察范围和管辖': { value: '第三章 监察范围和管辖', start: 15, end: 17 },
        '第四章 监察权限': { value: '第四章 监察权限', start: 18, end: 34 },
        '第五章 监察程序': { value: '第五章 监察程序', start: 35, end: 49 },
        '第六章 反腐败国际合作': { value: '第六章 反腐败国际合作', start: 50, end: 52 },
        '第七章 对监察机关和监察人员的监督': { value: '第七章 对监察机关和监察人员的监督', start: 53, end: 61 },
        '第八章 法律责任': { value: '第八章 法律责任', start: 62, end: 67 },
        '第九章 附则': { value: '第九章 附则', start: 68, end: 69 } },
    'police-regulation':{ '第一章 任务和基本原则': { value: '第一章 任务和基本原则', start: 1, end: 13 },
        '第二章 管辖': { value: '第二章 管辖', start: 14, end: 31 },
        '第三章 回避': { value: '第三章 回避', start: 32, end: 41 },
        '第四章 律师参与刑事诉讼': { value: '第四章 律师参与刑事诉讼', start: 42, end: 58 },
        '第五章 证据': { value: '第五章 证据', start: 59, end: 77 },
        '第六章 强制措施': { value: '第六章 强制措施', start: 78, end: 168 },
        '第一节 拘传': { value: '第一节 拘传', start: 78, end: 80 },
        '第二节 取保候审': { value: '第二节 取保候审', start: 81, end: 108 },
        '第三节 监视居住': { value: '第三节 监视居住', start: 109, end: 123 },
        '第四节 拘留': { value: '第四节 拘留', start: 124, end: 132 },
        '第五节 逮捕': { value: '第五节 逮捕', start: 133, end: 147 },
        '第六节 羁押': { value: '第六节 羁押', start: 148, end: 155 },
        '第七节 其他规定': { value: '第七节 其他规定', start: 156, end: 168 },
        '第七章 立案、撤案': { value: '第七章 立案、撤案', start: 169, end: 190 },
        '第一节 受案': { value: '第一节 受案', start: 169, end: 177 },
        '第二节 立案': { value: '第二节 立案', start: 178, end: 185 },
        '第三节 撤案': { value: '第三节 撤案', start: 186, end: 190 },
        '第八章 侦查': { value: '第八章 侦查', start: 191, end: 297 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 191, end: 197 },
        '第二节 讯问犯罪嫌疑人': { value: '第二节 讯问犯罪嫌疑人', start: 198, end: 209 },
        '第三节 询问证人、被害人': { value: '第三节 询问证人、被害人', start: 210, end: 212 },
        '第四节 勘验、检查': { value: '第四节 勘验、检查', start: 213, end: 221 },
        '第五节 搜查': { value: '第五节 搜查', start: 222, end: 226 },
        '第六节 查封、扣押': { value: '第六节 查封、扣押', start: 227, end: 236 },
        '第七节 查询、冻结': { value: '第七节 查询、冻结', start: 237, end: 247 },
        '第八节 鉴定': { value: '第八节 鉴定', start: 248, end: 257 },
        '第九节 辨认': { value: '第九节 辨认', start: 258, end: 262 },
        '第十节 技术侦查': { value: '第十节 技术侦查', start: 263, end: 273 },
        '第十一节 通缉': { value: '第十一节 通缉', start: 274, end: 282 },
        '第十二节 侦查终结': { value: '第十二节 侦查终结', start: 283, end: 294 },
        '第十三节 补充侦查': { value: '第十三节 补充侦查', start: 295, end: 297 },
        '第九章 执行刑罚': { value: '第九章 执行刑罚', start: 298, end: 316 },
        '第一节 罪犯的交付': { value: '第一节 罪犯的交付', start: 298, end: 304 },
        '第二节 减刑、假释、暂予监外执行': { value: '第二节 减刑、假释、暂予监外执行', start: 305, end: 310 },
        '第三节 剥夺政治权利': { value: '第三节 剥夺政治权利', start: 311, end: 314 },
        '第四节 对又犯新罪罪犯的处理': { value: '第四节 对又犯新罪罪犯的处理', start: 315, end: 316 },
        '第十章 特别程序': { value: '第十章 特别程序', start: 317, end: 345 },
        '第一节 未成年人刑事案件诉讼程序': { value: '第一节 未成年人刑事案件诉讼程序', start: 317, end: 332 },
        '第二节 当事人和解的公诉案件诉讼程序': { value: '第二节 当事人和解的公诉案件诉讼程序', start: 333, end: 338 },
        '第三节 犯罪嫌疑人逃匿、死亡案件违法所得的没收程序': { value: '第三节 犯罪嫌疑人逃匿、死亡案件违法所得的没收程序', start: 339, end: 341 },
        '第四节 依法不负刑事责任的精神病人的强制医疗程序': { value: '第四节 依法不负刑事责任的精神病人的强制医疗程序', start: 342, end: 345 },
        '第十一章 办案协作': { value: '第十一章 办案协作', start: 346, end: 356 },
        '第十二章 外国人犯罪案件的办理': { value: '第十二章 外国人犯罪案件的办理', start: 357, end: 373 },
        '第十三章 刑事司法协助和警务合作': { value: '第十三章 刑事司法协助和警务合作', start: 374, end: 384 },
        '第十四章 附则': { value: '第十四章 附则', start: 385, end: 388 } },
    'litigation-regulation': { '第一章 通则': { value: '第一章 通则', start: 1, end: 12 },
        '第二章 管辖': { value: '第二章 管辖', start: 13, end: 23 },
        '第三章 回避': { value: '第三章 回避', start: 24, end: 37 },
        '第四章 辩护与代理': { value: '第四章 辩护与代理', start: 38, end: 60 },
        '第五章 证据': { value: '第五章 证据', start: 61, end: 80 },
        '第六章 强制措施': { value: '第六章 强制措施', start: 81, end: 155 },
        '第一节 拘传': { value: '第一节 拘传', start: 81, end: 85 },
        '第二节 取保候审': { value: '第二节 取保候审', start: 86, end: 106 },
        '第三节 监视居住': { value: '第三节 监视居住', start: 107, end: 120 },
        '第四节 拘留': { value: '第四节 拘留', start: 121, end: 127 },
        '第五节 逮捕': { value: '第五节 逮捕', start: 128, end: 141 },
        '第六节 监察机关移送案件的强制措施': { value: '第六节 监察机关移送案件的强制措施', start: 142, end: 147 },
        '第七节 其他规定': { value: '第七节 其他规定', start: 148, end: 155 },
        '第七章 案件受理': { value: '第七章 案件受理', start: 156, end: 165 },
        '第八章 立案': { value: '第八章 立案', start: 166, end: 175 },
        '第一节 立案审查': { value: '第一节 立案审查', start: 166, end: 170 },
        '第二节 立案决定': { value: '第二节 立案决定', start: 171, end: 175 },
        '第九章 侦查': { value: '第九章 侦查', start: 176, end: 254 },
        '第一节 一般规定': { value: '第一节 一般规定', start: 621, end: 624 },
        '第二节 讯问犯罪嫌疑人': { value: '第二节 讯问犯罪嫌疑人', start: 182, end: 190 },
        '第三节 询问证人、被害人': { value: '第三节 询问证人、被害人', start: 191, end: 195 },
        '第四节 勘验、检查': { value: '第四节 勘验、检查', start: 196, end: 201 },
        '第五节 搜查': { value: '第五节 搜查', start: 202, end: 207 },
        '第六节 调取、查封、扣押、查询、冻结': { value: '第六节 调取、查封、扣押、查询、冻结', start: 208, end: 217 },
        '第七节 鉴定': { value: '第七节 鉴定', start: 218, end: 222 },
        '第八节 辨认': { value: '第八节 辨认', start: 223, end: 226 },
        '第九节 技术侦查措施': { value: '第九节 技术侦查措施', start: 227, end: 231 },
        '第十节 通缉': { value: '第十节 通缉', start: 232, end: 236 },
        '第十一节 侦查终结': { value: '第十一节 侦查终结', start: 237, end: 254 },
        '第十章 审查逮捕和审查起诉': { value: '第十章 审查逮捕和审查起诉', start: 255, end: 389 },
        '第二节 认罪认罚从宽案件办理': { value: '第二节 认罪认罚从宽案件办理', start: 267, end: 279 },
        '第三节 审查批准逮捕': { value: '第三节 审查批准逮捕', start: 280, end: 295 },
        '第四节 审查决定逮捕': { value: '第四节 审查决定逮捕', start: 296, end: 304 },
        '第五节 延长侦查羁押期限和重新计算侦查羁押期限': { value: '第五节 延长侦查羁押期限和重新计算侦查羁押期限', start: 305, end: 319 },
        '第六节 核准追诉': { value: '第六节 核准追诉', start: 320, end: 327 },
        '第七节 审查起诉': { value: '第七节 审查起诉', start: 328, end: 354 },
        '第八节 起诉': { value: '第八节 起诉', start: 355, end: 364 },
        '第九节 不起诉': { value: '第九节 不起诉', start: 365, end: 389 },
        '第十一章 出席法庭': { value: '第十一章 出席法庭', start: 390, end: 456 },
        '第一节 出席第一审法庭': { value: '第一节 出席第一审法庭', start: 390, end: 429 },
        '第二节 简易程序': { value: '第二节 简易程序', start: 430, end: 436 },
        '第三节 速裁程序': { value: '第三节 速裁程序', start: 437, end: 444 },
        '第四节 出席第二审法庭': { value: '第四节 出席第二审法庭', start: 445, end: 453 },
        '第五节 出席再审法庭': { value: '第五节 出席再审法庭', start: 454, end: 456 },
        '第十二章 特别程序': { value: '第十二章 特别程序', start: 457, end: 550 },
        '第一节 未成年人刑事案件诉讼程序': { value: '第一节 未成年人刑事案件诉讼程序', start: 457, end: 491 },
        '第二节 当事人和解的公诉案件诉讼程序': { value: '第二节 当事人和解的公诉案件诉讼程序', start: 492, end: 504 },
        '第三节 缺席审判程序': { value: '第三节 缺席审判程序', start: 505, end: 511 },
        '第四节 犯罪嫌疑人、被告人逃匿、死亡案件违法所得的没收程序':
            { value: '第四节 犯罪嫌疑人、被告人逃匿、死亡案件违法所得的没收程序', start: 512, end: 533 },
        '第五节 依法不负刑事责任的精神病人的强制医疗程序': { value: '第五节 依法不负刑事责任的精神病人的强制医疗程序', start: 534, end: 550 },
        '第十三章 刑事诉讼法律监督': { value: '第十三章 刑事诉讼法律监督', start: 551, end: 620 },
        '第二节 刑事立案监督': { value: '第二节 刑事立案监督', start: 557, end: 566 },
        '第三节 侦查活动监督': { value: '第三节 侦查活动监督', start: 567, end: 569 },
        '第四节 审判活动监督': { value: '第四节 审判活动监督', start: 570, end: 572 },
        '第五节 羁押必要性审查': { value: '第五节 羁押必要性审查', start: 573, end: 582 },
        '第六节 刑事判决、裁定监督': { value: '第六节 刑事判决、裁定监督', start: 583, end: 601 },
        '第七节 死刑复核监督': { value: '第七节 死刑复核监督', start: 602, end: 611 },
        '第八节 羁押期限和办案期限监督': { value: '第八节 羁押期限和办案期限监督', start: 612, end: 620 },
        '第十四章 刑罚执行和监管执法监督': { value: '第十四章 刑罚执行和监管执法监督', start: 621, end: 663 },
        '第二节 交付执行监督': { value: '第二节 交付执行监督', start: 625, end: 628 },
        '第三节 减刑、假释、暂予监外执行监督': { value: '第三节 减刑、假释、暂予监外执行监督', start: 629, end: 641 },
        '第四节 社区矫正监督': { value: '第四节 社区矫正监督', start: 642, end: 644 },
        '第五节 刑事裁判涉财产部分执行监督': { value: '第五节 刑事裁判涉财产部分执行监督', start: 645, end: 646 },
        '第六节 死刑执行监督': { value: '第六节 死刑执行监督', start: 647, end: 650 },
        '第七节 强制医疗执行监督': { value: '第七节 强制医疗执行监督', start: 651, end: 653 },
        '第八节 监管执法监督': { value: '第八节 监管执法监督', start: 654, end: 661 },
        '第九节 事故检察': { value: '第九节 事故检察', start: 662, end: 663 },
        '第十五章 案件管理': { value: '第十五章 案件管理', start: 664, end: 670 },
        '第十六章 刑事司法协助': { value: '第十六章 刑事司法协助', start: 671, end: 679 },
        '第十七章 附则': { value: '第十七章 附则', start: 680, end: 684 } },
    'police-law': { '第一章 总则': { value: '第一章 总则', start: 1, end: 5 },
        '第二章 职权': { value: '第二章 职权', start: 6, end: 19 },
        '第三章 义务和纪律': { value: '第三章 义务和纪律', start: 20, end: 23 },
        '第四章 组织管理': { value: '第四章 组织管理', start: 24, end: 31 },
        '第五章 警务保障': { value: '第五章 警务保障', start: 32, end: 41 },
        '第六章 执法监督': { value: '第六章 执法监督', start: 42, end: 57 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 48, end: 50 },
        '第八章 附则': { value: '第八章 附则', start: 51, end: 52 } },
    'police-enforce-detail':{ '第一章 一般规定': { value: '第一章 一般规定', start: 1, end: 6 },
        '第二章 管辖': { value: '第二章 管辖', start: 7, end: 14 },
        '第三章 立案': { value: '第三章 立案', start: 15, end: 22 },
        '第四章 回避': { value: '第四章 回避', start: 23, end: 26 },
        '第五章 律师参与刑事诉讼': { value: '第五章 律师参与刑事诉讼', start: 27, end: 28 },
        '第六章 勘验、检查': { value: '第六章 勘验、检查', start: 29, end: 44 },
        '第七章 计算机犯罪现场勘验与': { value: '第七章 计算机犯罪现场勘验与', start: 45, end: 50 },
        '第八章 搜查': { value: '第八章 搜查', start: 51, end: 54 },
        '第九章 扣押和调取证据': { value: '第九章 扣押和调取证据', start: 55, end: 57 },
        '第十章 鉴定': { value: '第十章 鉴定', start: 58, end: 67 },
        '第十一章 辨认': { value: '第十一章 辨认', start: 68, end: 72 },
        '第十二章 查询、冻结': { value: '第十二章 查询、冻结', start: 73, end: 74 },
        '第十三章 讯问犯罪嫌疑人': { value: '第十三章 讯问犯罪嫌疑人', start: 75, end: 84 },
        '第十四章 询问证人、被害人': { value: '第十四章 询问证人、被害人', start: 85, end: 91 },
        '第十五章 缉': { value: '第十五章 缉', start: 92, end: 97 },
        '第十六章 犯罪信息采集与网上侦查措施': { value: '第十六章 犯罪信息采集与网上侦查措施', start: 98, end: 101 },
        '第十七章 拘传': { value: '第十七章 拘传', start: 102, end: 105 },
        '第十八章 取保候审': { value: '第十八章 取保候审', start: 106, end: 112 },
        '第十九章 监视居住': { value: '第十九章 监视居住', start: 113, end: 118 },
        '第二十章 拘留': { value: '第二十章 拘留', start: 119, end: 127 },
        '第二十一章 逮捕': { value: '第二十一章 逮捕', start: 128, end: 139 },
        '第二十二章 羁押': { value: '第二十二章 羁押', start: 140, end: 145 },
        '第二十三章 对人大代表、政协委员、外国人、港澳台居民采取强制措施的特别规定':
            { value: '第二十三章 对人大代表、政协委员、外国人、港澳台居民采取强制措施的特别规定',
                start: 146,
                end: 149 },
        '第二十四章 办案协作': { value: '第二十四章 办案协作', start: 150, end: 155 },
        '第二十五章 审查判断证据': { value: '第二十五章 审查判断证据', start: 156, end: 167 },
        '第二十六章 侦查终结': { value: '第二十六章 侦查终结', start: 168, end: 172 },
        '第二十七章 补充侦查': { value: '第二十七章 补充侦查', start: 173, end: 175 },
        '第二十八章 管辖': { value: '第二十八章 管辖', start: 176, end: 178 },
        '第二十九章 简易程序(当场处罚)、现场调解': { value: '第二十九章 简易程序(当场处罚)、现场调解', start: 179, end: 180 },
        '第三十章 受案': { value: '第三十章 受案', start: 181, end: 187 },
        '第三十一章 回避': { value: '第三十一章 回避', start: 188, end: 191 },
        '第三十二章 询问': { value: '第三十二章 询问', start: 192, end: 193 },
        '第三十三章 勘验、检查': { value: '第三十三章 勘验、检查', start: 194, end: 195 },
        '第三十四章 扣押': { value: '第三十四章 扣押', start: 196, end: 201 },
        '第三十五章 先行登记保存': { value: '第三十五章 先行登记保存', start: 202, end: 206 },
        '第三十六章 抽样取证': { value: '第三十六章 抽样取证', start: 207, end: 211 },
        '第三十七章 鉴定、检测、检验': { value: '第三十七章 鉴定、检测、检验', start: 212, end: 223 },
        '第三十八章 辨认': { value: '第三十八章 辨认', start: 224, end: 226 },
        '第三十九章 治安调解': { value: '第三十九章 治安调解', start: 227, end: 234 },
        '第四十章 听证': { value: '第四十章 听证', start: 235, end: 241 },
        '第四十一章 决定行政处罚': { value: '第四十一章 决定行政处罚', start: 242, end: 252 },
        '第四十二章 处理涉案财物': { value: '第四十二章 处理涉案财物', start: 253, end: 257 },
        '第四十三章 执行': { value: '第四十三章 执行', start: 258, end: 265 },
        '第四十四章 案件终结': { value: '第四十四章 案件终结', start: 266, end: 269 } },
    'national-compensation':{ '第一章 总则': { value: '第一章 总则', start: 1, end: 2 },
        '第二章 行政赔偿': { value: '第二章 行政赔偿', start: 3, end: 16 },
        '第一节 赔偿范围': { value: '第一节 赔偿范围', start: 17, end: 19 },
        '第二节 赔偿请求人和赔偿义务机关': { value: '第二节 赔偿请求人和赔偿义务机关', start: 20, end: 21 },
        '第三节 赔偿程序': { value: '第三节 赔偿程序', start: 22, end: 31 },
        '第三章 刑事赔偿': { value: '第三章 刑事赔偿', start: 17, end: 31 },
        '第四章 赔偿方式和计算标准': { value: '第四章 赔偿方式和计算标准', start: 32, end: 37 },
        '第五章 其他规定': { value: '第五章 其他规定', start: 38, end: 40 },
        '第六章 附则': { value: '第六章 附则', start: 41, end: 42 } },
    'exit-entry-law':{ '第一章 总 则': { value: '第一章 总 则', start: 1, end: 8 },
        '第二章 中国公民出境入境': { value: '第二章 中国公民出境入境', start: 9, end: 14 },
        '第三章 外国人入境出境': { value: '第三章 外国人入境出境', start: 15, end: 28 },
        '第一节 签 证': { value: '第一节 签 证', start: 15, end: 23 },
        '第二节 入境出境': { value: '第二节 入境出境', start: 24, end: 28 },
        '第四章 外国人停留居留': { value: '第四章 外国人停留居留', start: 29, end: 49 },
        '第一节 停留居留': { value: '第一节 停留居留', start: 29, end: 46 },
        '第二节 永久居留': { value: '第二节 永久居留', start: 47, end: 49 },
        '第五章 交通运输工具出境入境边防检查': { value: '第五章 交通运输工具出境入境边防检查', start: 50, end: 57 },
        '第六章 调查和遣返': { value: '第六章 调查和遣返', start: 58, end: 69 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 70, end: 88 },
        '第八章 附 则': { value: '第八章 附 则', start: 89, end: 93 } },
    'help-law':{ '第一章　总　　则': { value: '第一章　总　　则', start: 1, end: 11 },
        '第二章　机构和人员': { value: '第二章　机构和人员', start: 12, end: 21 },
        '第三章　形式和范围': { value: '第三章　形式和范围', start: 22, end: 34 },
        '第四章　程序和实施': { value: '第四章　程序和实施', start: 35, end: 50 },
        '第五章　保障和监督': { value: '第五章　保障和监督', start: 51, end: 60 },
        '第六章　法律责任': { value: '第六章　法律责任', start: 61, end: 67 },
        '第七章　附　　则': { value: '第七章　附　　则', start: 68, end: 71 } },
    'consumer-right-protect-law': { '第一章 总　则': { value: '第一章 总　则', start: 1, end: 6 },
        '第二章 消费者的权利': { value: '第二章 消费者的权利', start: 7, end: 15 },
        '第三章 经营者的义务': { value: '第三章 经营者的义务', start: 16, end: 29 },
        '第四章 国家对消费者合法权益的保护': { value: '第四章 国家对消费者合法权益的保护', start: 30, end: 35 },
        '第五章 消费者组织': { value: '第五章 消费者组织', start: 36, end: 38 },
        '第六章 争议的解决': { value: '第六章 争议的解决', start: 39, end: 47 },
        '第七章 法律责任': { value: '第七章 法律责任', start: 48, end: 61 },
        '第八章 附　则': { value: '第八章 附　则', start: 62, end: 63 } },
    'admin-reconsider-law-2024': { '第一章　总则': { value: '第一章　总则', start: 1, end: 10 },
        '第二章　行政复议申请': { value: '第二章　行政复议申请', start: 11, end: 29 },
        '第一节　行政复议范围': { value: '第一节　行政复议范围', start: 11, end: 13 },
        '第二节　行政复议参加人': { value: '第二节　行政复议参加人', start: 14, end: 19 },
        '第三节　申请的提出': { value: '第三节　申请的提出', start: 20, end: 23 },
        '第四节　行政复议管辖': { value: '第四节　行政复议管辖', start: 24, end: 29 },
        '第三章　行政复议受理': { value: '第三章　行政复议受理', start: 30, end: 35 },
        '第四章　行政复议审理': { value: '第四章　行政复议审理', start: 36, end: 60 },
        '第一节　一般规定': { value: '第一节　一般规定', start: 36, end: 42 },
        '第二节　行政复议证据': { value: '第二节　行政复议证据', start: 43, end: 47 },
        '第三节　普通程序': { value: '第三节　普通程序', start: 48, end: 52 },
        '第四节　简易程序': { value: '第四节　简易程序', start: 53, end: 55 },
        '第五节　行政复议附带审查': { value: '第五节　行政复议附带审查', start: 56, end: 60 },
        '第五章　行政复议决定': { value: '第五章　行政复议决定', start: 61, end: 79 },
        '第六章　法律责任': { value: '第六章　法律责任', start: 80, end: 86 },
        '第七章　附则': { value: '第七章　附则', start: 87, end: 90 } },
    'civil-law-regulation-2024': { '第一章　任务、适用范围和基本原则': { value: '第一章　任务、适用范围和基本原则', start: 1, end: 17 },
        '第二章　管 辖': { value: '第二章　管 辖', start: 18, end: 39 },
        '第一节　级别管辖': { value: '第一节　级别管辖', start: 18, end: 21 },
        '第二节　地域管辖': { value: '第二节　地域管辖', start: 22, end: 36 },
        '第三节　移送管辖和指定管辖': { value: '第三节　移送管辖和指定管辖', start: 37, end: 39 },
        '第三章　审判组织': { value: '第三章　审判组织', start: 40, end: 46 },
        '第四章　回 避': { value: '第四章　回 避', start: 47, end: 50 },
        '第五章　诉讼参加人': { value: '第五章　诉讼参加人', start: 51, end: 65 },
        '第一节　当事人': { value: '第一节　当事人', start: 51, end: 59 },
        '第二节　诉讼代理人': { value: '第二节　诉讼代理人', start: 60, end: 65 },
        '第六章　证 据': { value: '第六章　证 据', start: 66, end: 84 },
        '第七章　期间、送达': { value: '第七章　期间、送达', start: 85, end: 95 },
        '第一节　期 间': { value: '第一节　期 间', start: 85, end: 86 },
        '第二节　送 达': { value: '第二节　送 达', start: 87, end: 95 },
        '第八章　调 解': { value: '第八章　调 解', start: 96, end: 102 },
        '第九章　保全和先予执行': { value: '第九章　保全和先予执行', start: 103, end: 111 },
        '第十章　对妨害民事诉讼的强制措施': { value: '第十章　对妨害民事诉讼的强制措施', start: 112, end: 120 },
        '第十一章　诉讼费用': { value: '第十一章　诉讼费用', start: 121, end: 121 },
        '第十二章　第一审普通程序': { value: '第十二章　第一审普通程序', start: 122, end: 159 },
        '第一节　起诉和受理': { value: '第一节　起诉和受理', start: 122, end: 127 },
        '第二节　审理前的准备': { value: '第二节　审理前的准备', start: 128, end: 136 },
        '第三节　开庭审理': { value: '第三节　开庭审理', start: 137, end: 152 },
        '第四节　诉讼中止和终结': { value: '第四节　诉讼中止和终结', start: 153, end: 154 },
        '第五节　判决和裁定': { value: '第五节　判决和裁定', start: 155, end: 159 },
        '第十三章　简易程序': { value: '第十三章　简易程序', start: 160, end: 170 },
        '第十四章　第二审程序': { value: '第十四章　第二审程序', start: 171, end: 183 },
        '第十五章　特别程序': { value: '第十五章　特别程序', start: 184, end: 208 },
        '第一节　一般规定': { value: '第一节　一般规定', start: 184, end: 187 },
        '第二节　选民资格案件': { value: '第二节　选民资格案件', start: 188, end: 189 },
        '第三节　宣告失踪、宣告死亡案件': { value: '第三节　宣告失踪、宣告死亡案件', start: 190, end: 193 },
        '第四节　指定遗产管理人案件': { value: '第四节　指定遗产管理人案件', start: 194, end: 197 },
        '第五节　认定公民无民事行为能力、限制民事行为能力案件': { value: '第五节　认定公民无民事行为能力、限制民事行为能力案件', start: 198, end: 201 },
        '第六节　认定财产无主案件': { value: '第六节　认定财产无主案件', start: 202, end: 204 },
        '第七节　确认调解协议案件': { value: '第七节　确认调解协议案件', start: 205, end: 206 },
        '第八节　实现担保物权案件': { value: '第八节　实现担保物权案件', start: 207, end: 208 },
        '第十六章　审判监督程序': { value: '第十六章　审判监督程序', start: 209, end: 224 },
        '第十七章　督促程序': { value: '第十七章　督促程序', start: 225, end: 228 },
        '第十八章　公示催告程序': { value: '第十八章　公示催告程序', start: 229, end: 234 },
        '第十九章　一般规定': { value: '第十九章　一般规定', start: 235, end: 246 },
        '第二十章　执行的申请和移送': { value: '第二十章　执行的申请和移送', start: 247, end: 251 },
        '第二十一章　执行措施': { value: '第二十一章　执行措施', start: 252, end: 266 },
        '第二十二章　执行中止和终结': { value: '第二十二章　执行中止和终结', start: 267, end: 269 },
        '第二十三章　一般原则': { value: '第二十三章　一般原则', start: 270, end: 275 },
        '第二十四章　管 辖': { value: '第二十四章　管 辖', start: 276, end: 282 },
        '第二十五章　送达、调查取证、期间': { value: '第二十五章　送达、调查取证、期间', start: 283, end: 287 },
        '第二十六章　仲 裁': { value: '第二十六章　仲 裁', start: 288, end: 292 },
        '第二十七章　司法协助': { value: '第二十七章　司法协助', start: 293, end: 306 } }
}

exports.main = async (event, context) => {
    console.log(event)
    const db = cloud.database()
    const _ = db.command

    const {type, law, searchValue}= event

    const numberMap = lawMap[law]

    if (!isNaN(parseInt(searchValue))) {
        return await db.collection('other-law')
            .where({
                number: parseInt(searchValue),
                type: law
            }).orderBy('number', 'asc').limit(1000).get()
    }

    if (type === 'category') {
        const {start, end} = numberMap[searchValue]
        return await db.collection('other-law').where({
            number: _.and(_.gte(start),_.lte(end)),
            type: law
        }).orderBy('number', 'asc').limit(1000).get()
    }

    return await db.collection('other-law')
        .where({
            text: db.RegExp({
                regexp: '.*' + searchValue,
                options: 'i',
            }),
            type: law
        }).orderBy('number', 'asc').limit(1000).get()


}
